...
基于 OpenAI、OpenAI 兼容转写接口和 Google Gemini，使用 Tauri v2 构建。
...
- **波形浮窗** — 录音时显示极简的置顶波形动画。
- **并发转写** — 上一段还在转写时即可开始下一段录音，会话在浮窗中向上堆叠，每段结果就绪后立即粘贴。
- **历史记录** — 所有转写结果按说话顺序本地保存，附带音频文件可重试。
- **服务商设置** — 支持 OpenAI、Gemini 或 OpenAI 兼容接口。
- **网络设置** — 支持系统代理、无代理或自定义 HTTP/SOCKS 代理。
- **系统托盘** — 安静地驻留后台。
